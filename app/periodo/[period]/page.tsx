 ...
 *
 * El flujo es el de la operativa real: arrastrar documentos → procesar →
 * revisar. Lo primero que se ve tras la carga es lo que hay que hacer (la cola
 * de revisión), y debajo todos los movimientos con su documento asociado, que
 * se puede abrir sin salir de aquí.
 */
...
export default async function PeriodPage({
  ...
  const [ledger, registry] = await Promise.all([
    loadPeriodLedger(period),
    loadPeriodDocuments(period),
  ]);

  const pendingDocuments = registry.documents
    .filter((document) => document.needsReview)
    .map((document) => ({
      hash: document.hash,
  ...
  return (
    <main className="mx-auto w-full max-w-6xl flex-1 space-y-10 px-6 py-10">
      <header className="space-y-1">
        ...
        <h1 className="text-2xl font-semibold tracking-tight capitalize">{label}</h1>
        <p className="text-sm text-neutral-600 dark:text-neutral-400">
          {registry.documents.length} documento{registry.documents.length === 1 ? "" : "s"} subido
          {registry.documents.length === 1 ? "" : "s"}
          {view ? ` · ${ledger!.summary.entryCount} movimientos · ${view.metrics.reconciledPct}% conciliado` : " · sin procesar"}
        </p>
      </header>

      {/* ── Carga de documentos ─────────────────────────────────────────── */}
      <section className="space-y-4">
        <DropZone period={period} periodLabel={label} />

        <details className="rounded-lg border border-neutral-200 px-4 py-3 text-sm dark:border-neutral-800">
          <summary className="cursor-pointer font-medium">Añadir extractos</summary>
          <div className="pt-4">
            <p className="mb-3 text-neutral-600 dark:text-neutral-400">
              Los extractos también se reconocen desde la zona de arriba. Usa esto solo si
              quieres indicar la cuenta por adelantado.
            </p>
            <DropZone period={period} periodLabel={label} mode="statements" />
          </div>
        </details>

        <ProcessButton period={period} />
      </section>
      ...
      {!view && (
        <p className="rounded-lg border border-neutral-200 p-4 text-sm text-neutral-600 dark:border-neutral-800 dark:text-neutral-400">
          Este mes todavía no se ha procesado. Arrastra los documentos y pulsa
          <strong> Procesar mes</strong>.
        </p>
      )}
      ...
      {view && (
        <>
          ...
          <section className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            <Metric label="Ingresos" value={formatCents(view.metrics.incomeCents)} />
            <Metric label="Gastos" value={formatCents(view.metrics.expenseCents)} />
            <Metric label="Flujo neto" value={formatCents(view.metrics.netCashFlowCents)} />
            <Metric label="Conciliado" value={`${view.metrics.reconciledPct}%`} />
            <Metric label="Pendientes" value={String(view.metrics.pendingCount)} />
            <Metric label="Sin justificar" value={formatCents(view.metrics.unjustifiedAmountCents)} />
          </section>
            ...
            <div className="overflow-x-auto rounded-lg border border-neutral-200 dark:border-neutral-800">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                  {view.metrics.byAccount.map((account) => (
          ...
          <section className="space-y-3">
                        ...
                        <Td>
                          {row.documentName === null ? (
                            ...
                            <a
                              ...
                              title="Abrir el documento justificativo"
                            >
                              {row.documentName}
                            </a>
                          ) : (
            ...
            <p className="text-xs text-neutral-500">
              La clasificación (categoría y P&amp;L) se diseñará en una fase posterior.
            </p>
          </section>
        </>
      )}
